...
function getToolLabel(toolName: string, args: Record<string, unknown>, state: string): string {
  ...
  if (toolName === "str_replace_editor") {
    ...
    switch (command) {
      case "create":   return done ? `Created${fileLabel}` : `Writing${fileLabel}…`;
      case "str_replace":
      case "insert":   return done ? `Updated${fileLabel}` : `Updating${fileLabel}…`;
      case "view":     return done ? `Read${fileLabel}` : `Reading${fileLabel}…`;
      default:         return done ? `Edited${fileLabel}` : `Editing${fileLabel}…`;
    }
  }
  ...
  if (toolName === "file_manager") {
    const command = args?.command as string | undefined;
    switch (command) {
      case "delete": return done ? `Removed${fileLabel}` : `Removing${fileLabel}…`;
      case "rename": return done ? `Renamed${fileLabel}` : `Renaming${fileLabel}…`;
      default:       return done ? `File operation done` : `Working on files…`;
    }
  }
